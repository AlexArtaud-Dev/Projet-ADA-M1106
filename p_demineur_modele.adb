...
function VictoireJoueur (G : in TV_Grille ) return Boolean is
  ...
  begin
	ligne := G'First(1);
	colonne := G'First(2);
	while victoire and G(ligne, colonne) /= G(G'Last(1), G'Last(2) loop
		ligne := null;
		while victoire and G(ligne, colonne) /= G(G'Last(1), G'Last(2) loop
			colonne := null;
			if not G(ligne, colonne).Occupee and G(ligne, colonne).Etat = Decouverte then
				victoire := false;
			end if;
		end loop;
	end loop;

	if(G(ligne, colonne) = G(G'Last(1), G'Last(2))) then
		return not G(ligne, colonne).Occupee and G(ligne, colonne).Etat = montree;
	else
...
---------------------------------------------------------------------------------------------------------------------------------------------
 ...
 -- victoire : boolean := true;  
 -- begin
--	for ligne in G'first(1)..G'last(1) loop -- Check par ligne les cases (link avec le check du dessous)
--	  for colonne in G'first(2)..G'last(2) loop -- Check par colonne les cases (link avec le che  ck du dessus)
--		if not G(ligne, colonne).Occupee and G(ligne, colonne).Etat = couverte then
--victoire := false; -- Si une position est occupée alors le joueur a perdu
--		end if;
--	  end loop;
--	end loop;
--	return victoire; -- Return si le joueur a gagné 
 -- end VictoireJoueur;
